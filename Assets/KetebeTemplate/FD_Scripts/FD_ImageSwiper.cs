...
{
    public class FD_ImageSwiper : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            { // sol tıklandı mı kontrol ediliyor
                ...
                { // en az bir nesne bulunduysa
                    ...
                    {
                        if (hit.collider.GetComponent<FD_Types>() != null)
                        {
                            if (hit.collider.GetComponent<FD_Types>().FDPage != Pages.FDPages)
                            {
                                isDraggable = false;
                    ...
                    { // tüm nesneleri döngüye sokuyoruz
                        ...
                        if (hit.collider.GetComponent<FD_Types>() != null)
                        {
                            if (hits[0].collider.GetComponent<FD_Types>() != null)
                            {

                                if (hit.collider.GetComponent<FD_Types>().FDPage == Pages.FDPages && hits[0].collider.GetComponent<FD_Types>().FDPage == Pages.FDPages && isDraggable)
                                {
